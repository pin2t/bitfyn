...
func main() {
	...
	var entropy, err = bip39.EntropyFromMnemonic(mnemonic)
	must(err)
	var h = sha256.Sum256(entropy)
	...
	var mac = hmac.New(sha512.New, []byte("Bitcoin seed"))
	mac.Write(seed)
	var I = mac.Sum(nil)
	var IL, IR = I[:32], I[32:]
	...
	fmt.Printf("master zprv (SLIP-132)       : %s\n", toVersion(master.String(), zprvVer))
	fmt.Printf("  matches BIP84 rootpriv     : %v\n", toVersion(master.String(), zprvVer) == specRootPriv)
	npub, err := master.Neuter()
	must(err)
	var mzpub = toVersion(npub.String(), zpubVer)
	fmt.Printf("master zpub (SLIP-132)       : %s\n  matches BIP84 rootpub      : %v\n", mzpub, mzpub == specRootPub)
	...
	for n, s := range steps {
		var child = hdkeychain.HardenedKeyStart + s.idx
		var _, _, pChain, pPriv = decodeExt(key.String())
		key, err = key.Derive(child)
		must(err)
		var depth, _, chain, priv = decodeExt(key.String())
		fmt.Printf("\n-- derive %s' (child %d = 2^31+%d) --\n", s.name, child, s.idx)
		if n == 0 {
			var data = append([]byte{0x00}, pPriv...)
			data = append(data, ser32(child)...)
			var hm = hmac.New(sha512.New, pChain)
			hm.Write(data)
			var out = hm.Sum(nil)
		...
		var neutered, err = key.Neuter()
		must(err)
		var zp = toVersion(neutered.String(), zpubVer)
		switch s.name {
		case "purpose":
			fmt.Printf("   m/84'   zpub: %s\n", zp)
		case "coin_type":
			fmt.Printf("   m/84'/0' zpub: %s\n", zp)
		}
		fmt.Printf("   equals BIP84 rootpub? %v\n", zp == specRootPub)
	}
	fmt.Printf("\naccount xprv (m/84'/0'/0')    : %s\n", key.String())
	var zprv = toVersion(key.String(), zprvVer)
	...
	var zpub = toVersion(neutered.String(), zpubVer)
	...
	for i, idx := range []uint32{0, 1} {
		var changeKey, err = watch.Derive(externalChain)
		must(err)
		addrKey, err := changeKey.Derive(idx)
		must(err)
		pub, err := addrKey.ECPubKey()
		must(err)
		var comp = pub.SerializeCompressed()
		watchPubs[i] = comp
		var hash = btcutil.Hash160(comp)
		addr, err := btcutil.NewAddressWitnessPubKeyHash(hash, net)
		must(err)
		fmt.Printf("m/84'/0'/0'/0/%d: pubkey=%x  hash160=%x  address=%s\n", idx, comp, hash, addr)
	}
	changeKey, err := watch.Derive(changeChain)
	must(err)
	addrKey, err := changeKey.Derive(0)
	must(err)
	pub, err := addrKey.ECPubKey()
	must(err)
	var hash = btcutil.Hash160(pub.SerializeCompressed())
	...
	for _, idx := range []uint32{0, 1} {
		var addr, path, pubkey, err = w.DeriveAddress(idx)
		must(err)
...
func deriveAddr(net *chaincfg.Params, comp []byte) string {
	var a, err = btcutil.NewAddressWitnessPubKeyHash(btcutil.Hash160(comp), net)
	must(err)
	return a.String()
}
...
func toVersion(key string, newVersion [4]byte) string {
	...
	var payload = raw[4:78]
	var out = make([]byte, 0, 82)
	out = append(out, newVersion[:]...)
	out = append(out, payload...)
	var h1 = sha256.Sum256(out)
	var h2 = sha256.Sum256(h1[:])
	out = append(out, h2[:4]...)
